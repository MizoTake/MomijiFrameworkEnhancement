    ...
    private IEnumerator Roop (Vector3 blockScale) {
        ...
        while (true) {
            ...
            for (int i = 0; i < centers.Count; i++) {
                ...
                if (_normal) result.Append (centers[i].transform.DOLocalRotate (vectors[i] * -90, 0.0f, RotateMode.LocalAxisAdd));
            }
            // .Join(scaleAnim)
